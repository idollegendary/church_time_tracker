...
export default function ChurchPreachersModal({ open = false, church = null, preachers = [], sessions = [], onClose = ()=>{} }){
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-2xl bg-white dark:bg-neutral-900 rounded-lg shadow-lg overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold">{church.name}</h3>
            <div className="text-sm text-muted">Preachers</div>
          </div>
          <button onClick={onClose} className="p-1 rounded hover:bg-surface">✕</button>
        </div>
        <div className="p-4 space-y-3">
          {rows.length === 0 ? (
            <div className="text-sm text-muted">No preachers yet</div>
          ) : (
            rows.map(({ preacher, sessionsCount, totalSec }) => (
              <div key={preacher.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted">Sessions: {sessionsCount}</div>
                </div>
                <div className="text-sm text-muted flex-shrink-0">{formatDuration(totalSec)}</div>
              </div>
            ))
          )}
        </div>
